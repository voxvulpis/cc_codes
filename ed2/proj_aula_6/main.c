...
void searchByKey() {
    ...
    FILE* search_last_key = fopen("./last_access_search.bin", "r+b");
    if (!search_last_key) {
        printf("Arquivo de dados não encontrado. Criando um novo...\n");
        search_last_key = fopen("./last_access_search.bin", "wb");
        if (!search_last_key) {
            handleError("Erro ao criar o arquivo de dados.");
            return;
        }
        int zero = 0;
        fwrite(&zero, sizeof(int), 1, search_last_key);
    }

    fread(&last, sizeof(int), 1, search_last_key);
    fclose(search_last_key); // Fecha o arquivo após criá-lo
    ...
    char key[9];  // Para armazenar a chave (ID + Sigla)

    // Lê as chaves do arquivo de busca_p.bin
    for (int i = 0; i < numKeys + last; i++) {
        // Le tanto o ID quanto a Sigla do arquivo
        char id[5];    // ID de 4 caracteres + terminador
        char sigla[4]; // Sigla de 3 caracteres + terminador
        
        fread(id, sizeof(char), 4, searchFile); // Lê o ID
        id[4] = '\0'; // Garantir que o ID é uma string válida

        fread(sigla, sizeof(char), 4, searchFile); // Lê a sigla
        ...
        for (int j = 0; j < numPrimaryIndexEntries; j++) {
            if (strcmp(primaryIndex[j].key, key) == 0) {
                ...
                // Ignora o tamanho do registro (recordLength)
                char recordLength;
                fread(&recordLength, sizeof(char), 1, file);
                ...
                hist foundRecord;
                sscanf(record, "%[^#]#%[^#]#%[^#]#%[^#]#%f#%f",
                    foundRecord.id_aluno,    // ID Aluno
                    foundRecord.sigla_disc,  // Sigla Disciplina
                    foundRecord.nome_aluno,  // Nome Aluno
                    foundRecord.nome_disc,   // Nome Disciplina
                    &foundRecord.media,      // Média
                    &foundRecord.freq);      // Frequência
    ...
    search_last_key = fopen("last_access_search.bin", "r+b");
    last += numKeys;

    fwrite(&last, sizeof(int), 1, search_last_key);
    fclose(search_last_key);
}
    ...
    for (int i = 0; i < numNames; i++) {
        ...
        for (int j = 0; j < numSecondaryIndexEntries; j++) {
            if (strcmp(secondaryIndex[j].nome_aluno, name) == 0) {
                ...
                FILE *pri = fopen(PRIMARY_INDEX_FILE, "rb");
                if (!pri) {
                    handleError("Erro ao abrir o arquivo de índice secundário.");
                    return;
                }
                int k = 0;
                char key1[4];
                ...
                while (currentOffset != -1) {
                    ...
                    fread(&linkedEntry, sizeof(secondaryLinkedListEntry), 1, pri);
